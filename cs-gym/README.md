...
```bash
python3 cs-gym/serve.py
```

Then open <http://localhost:5177>. The first load pulls Pyodide (~10 MB) from
...
Every edit is written to `cs-gym/progress.json` about half a second after you
...
to `cs-gym/progress.json`. To start over, delete `progress.json` and run
